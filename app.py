...
picker.onchange=()=>{setStatus(mode);result.classList.add('hidden')};
function setStatus(text,warn){const el=document.querySelector('#corpus-label');el.textContent=currentTicker()+' - '+text;el.classList.toggle('stale',!!warn)}
